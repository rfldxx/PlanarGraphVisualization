...
function recalc_wait_time() {
    ...
    vertex_mention[zero_time_vertex] = [{x : 400, y :  50}];
...
function draw_segment(x1, y1, x2, y2, clr = COLORS.EDGE) {
    ctx.beginPath();
    ctx.moveTo(x1, y1);
    ctx.lineTo(x2, y2);
    ctx.strokeStyle = clr;
...
function draw_circle(x, y, r = VERTEX_RADIUS, clr = COLORS.VERTEX) {
    ctx.beginPath();
...
function draw_vertex(i) {
    ...
    let can_move_up = lst.length <= 2 ? dY: 0;
    ...
    vertex[i] = {x: x_mean, y: maxY + dY, x1: vertex_bounds[i].x1, x2: vertex_bounds[i].x2};
    ...
    for(let xy of lst) {
        ...
        draw_segment(xy.x, Math.max(xy.y, maxY), x, y, COLORS.VERTEX_SELECTED);
    ...
    for(let j = 0; j < n; j++) {
        ...
        vertex_mention[nxt].push({x: xj, y: yj});
