...
function draw_rect(x1, x2, y1, y2, t = 0) {
    if( !t ) ctx.fillStyle = 'rgba(127, 212, 101, 0.2)';
    else     ctx.fillStyle = 'rgba(247, 71, 127, 0.2)';
...
function draw_vertex(i) {
    ...
    all_dots.push( {x: x, y: y, dy: 0, comment: `vertex ${i}`} );
    ...
    for(let j = left_border-1; j >= 0; j--) {
        ...
        all_dots[id_in_all].y   = new_y;
        all_dots[id_in_all].dy += 0 + dy_up + dy_down;
    ...
    for(let j = right_border; j < to_optimize_lst.length; j++) {
        ...
        if( ans_x >= zone_x2 ) continue;  // ребро не оптимизируется
    ...
    for(let j = 0; j < n; j++) {
        ...
        all_dots.push( {x: xj, y: yj, dy: 0, comment: `mention ${nxt} from ${i}`} );  // пока dy = 0, длинна появится, когда эта ребро будет продленно вниз
