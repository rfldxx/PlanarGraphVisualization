...
// везде есть .id_in_all
let vertex_mention = [];
let vertex_bounds  = [];
let vertex = [{x: 250, y: 50, x1: 50, x2: 450}];  // а с ней что?? тут то нет .id_in_all?? Ладно пока не стреляет такчто забьём


// ай ладно, сделаем отдельный массив (по идеи эту информацию можно было получить из vertex и vertex_mention)
let all_dots = []; // {x, y1, y2, comment}, // [[удаленно! | dy - для вертикальных отрезков]]
...
let cnt_rect = 0;
function draw_rect(x1, x2, y1, y2, t = 0) {
    // t == 0 - область на право
    // t == 1 - область на лево
    ctx.fillStyle = t ? 'rgba(247,  71, 127, 0.2)' : 'rgba(127, 212, 101, 0.2)';
    ...
    if( !t ) ctx.fillText(`${cnt_rect}`, x1+5, y1+5);                   // координаты: отступ 5px от левого и верхнего края прямоугольника
    else     ctx.fillText(`${cnt_rect}`, x2-5, y1+5);     
    cnt_rect++;

    let separate_x = t ? x2-min_dX : x1+min_dX; 
    draw_segment(separate_x, y1, separate_x, y2); 
}
...
function draw_vertex(i) {
    ...
    all_dots.push( {x: x, y1: y, y2: y, comment: `vertex ${i}`} );
    ...
    for(let pos in lst) {
        let xx = lst[pos].x, yy = lst[pos].y;

        if( yy <= maxY ) {  // РЕБРО С ВЕРТИКАЛЬНОЙ ЧАСТЬЮ
            to_optimize_lst.push( {...lst[pos], pos_in_lst: pos} );
            continue;
        }
        
        if( xx == x ) continue;  // не надо перерисовывать - уже всё впорядке с ребром

        let prev = lst[pos].from;
        if( prev == -1 ) continue;
        
        // стираем прошлое ребро
        draw_segment(xx, yy, vertex[prev].x, vertex[prev].y, COLORS.BACKGROUND, 3);
        draw_circle( xx, yy, 5, COLORS.BACKGROUND, COLORS.BACKGROUND, 3);
        // 9
        ...
        // меняем координаты и в all_dots
        all_dots[lst[pos].id_in_all].x  = x;
        all_dots[lst[pos].id_in_all].y1 = y;
        all_dots[lst[pos].id_in_all].y2 = y;
    }
    ...
    to_optimize_lst.sort((a, b) => a.x-b.x);
    let left_border = 0;
    ...
    for(let j = left_border-1; j >= 0; j--) {
        let prev = to_optimize_lst[j].from;
        if( prev == -1 ) { console.log('ТАКОЕ БЫВАЕТ!!!'); continue; }
        if( vertex[prev].x <= to_optimize_lst[j].x ) {
            // такие рерба не улучшаем (хотя надо - там аналогичная область, только вдругую сторону)

            all_dots[to_optimize_lst[j].id_in_all].y2 = maxY;
            continue;
        }
        ...
        draw_rect(zone_x1, zone_x2, zone_y1, zone_y2);

        
        let ans_x = zone_x2;
        for(let xydy of all_dots) {  // перебор всех вершин
            let curx = xydy.x, cury1 = xydy.y1, cury2 = xydy.y2;
            // curx  == zone_x1  - это типо само ребро будет
            if( curx  <= zone_x1 || zone_x2 < curx  ) continue;
            if( cury2 <  zone_y1 || zone_y2 < cury1 ) continue;

            draw_circle(curx, cury1, 7, COLORS.RED, COLORS.RED);
            draw_circle(curx, cury2, 7, COLORS.RED, COLORS.RED);

            console.log("   account: [", xydy.comment, '] ', curx, cury1, cury2);
        ...
        console.log('!!!!!!AMAZZZINGGG!!!:  ', j, "    ???????");
        console.log(ans_x, '  =?= ', zone_x1);        

        if( ans_x <= zone_x1 ) {  // ребро не оптимизируется
            // -> y2 = maxY ???????????????
            all_dots[to_optimize_lst[j].id_in_all].y2 = maxY;
            continue;
        }

        console.log('!!!!!!AMAZZZINGGG!!!!!!!!!!'); 

        // эххх, жара.... парилка (P.S.: ммм Перлика)
        // ВСЁ УСТАЛ, ПОЙДУ В ARKNIGHTS ПОИГРАЮ И СПАТЬ
        let x_up   = vertex[prev].x,       y_up    = vertex[prev].y;
        let x_down = x,                    y_down  = y;
        let x_edge = to_optimize_lst[j].x, y_edge1 = to_optimize_lst[j].y, y_edge2 = maxY;
        
        let new_x = ans_x;
        
        let dy_up   = (y_up   - y_edge1) / (x_up   - x_edge) * (new_x - x_edge);
        let dy_down = (y_down - y_edge2) / (x_down - x_edge) * (new_x - x_edge);

        // ПРОСТИ математика, мне хочется плакать из-за этого костыля (=-=)
        // dy_up   = -Math.abs(dy_up   );
        // dy_down = Math.abs(dy_down);
        // во славу 5-го класса, это оказалось не нужным
        
        let new_y = to_optimize_lst[j].y + dy_up;


        // меняем координаты упоминания
        let id_in_all = to_optimize_lst[j].id_in_all;
        all_dots[id_in_all].x  = new_x;
        all_dots[id_in_all].y1 = new_y;
        all_dots[id_in_all].y2 = maxY + dy_down;
        // if( dy_up   < 0 ) console.log("\n!!!!!!!!!!!!!!WTFFFFFF UP");
        // if( dy_down < 0 ) console.log("\n!!!!!!!!!!!!!!WTFFFFFF DOWN");

        let old_x = to_optimize_lst[j].x, old_y = to_optimize_lst[j].y;
        to_optimize_lst[j].x = new_x;
        to_optimize_lst[j].y = new_y;

        // И В ИСХОДНОМ vertex_mention[i]; ( ай ладно, просто добавим ещё одно поле... :( )
        // let pos_in_lst = to_optimize_lst[j].pos_in_lst;
        // lst[pos_in_lst].x =  СТОП, А ЗАЧЕМ Я ЭТО ХОТЕЛ ДЕЛАТЬ????
        // ОКАЗЫВЕТСЯ ТУТ И ТАК ВСЁ ПРЕКРАСНО!!!
        // ДАЖЕ В to_optimize_lst НЕ НАДО МЕНЯТЬ КООРДИНАТЫ



        // затираем часть ребра
        // стираем прошлое ребро
        draw_segment(new_x, new_y, old_x, old_y, COLORS.BACKGROUND, 3);
        draw_circle(old_x, old_y, 5, COLORS.BACKGROUND, COLORS.BACKGROUND, 3);
        draw_vertex_label(prev);
    }



    // ПОЛНЫЙ АД - ПОЛНОСТЬЮ КОПИРОВАТЬ ДЛЯ ПРАВОЙ СТОРОНЫ ,,,,,,,,,,,,,,,
    // вертикализуем "ПРАВОЙ" ребра
    let cnt_chk_right = 0;
    for(let j = right_border; j < to_optimize_lst.length; j++) {
        let prev = to_optimize_lst[j].from;
        if( prev == -1 ) { console.log('ТАКОЕ БЫВАЕТ!!!'); continue; }
        if( vertex[prev].x >= to_optimize_lst[j].x ) {
            // такие рерба не улучшаем (хотя надо - там аналогичная область, только вдругую сторону)
        ...
        // search zone:  x: [zone_x1 zone_x2]
        let zone_x1 = Math.max(x, vertex[prev].x) - min_dX, zone_x2 = to_optimize_lst[j].x;
        let zone_y1 = to_optimize_lst[j].y, zone_y2 = maxY;
        draw_rect(zone_x1, zone_x2, zone_y1, zone_y2, 1);

        
        let ans_x = zone_x1;
        for(let xydy of all_dots) {  // перебор всех вершин
            let curx = xydy.x, cury1 = xydy.y1, cury2 = xydy.y2;
            // curx  == zone_x2  - это типо само ребро будет
            if( curx  < zone_x1 || zone_x2 <= curx  ) continue;
            if( cury2 <  zone_y1 || zone_y2 < cury1 ) continue;
            ...
            console.log("   account: [", xydy.comment, '] ', curx, cury1, cury2);
            ans_x = Math.max(ans_x, curx);
        }

        ans_x += min_dX;

        if( ans_x >= zone_x2 ) {  // ребро не оптимизируется
            // -> y2 = maxY ???????????????
            all_dots[to_optimize_lst[j].id_in_all].y2 = maxY;
            continue;
        }
        ...
        // эххх, жара.... парилка
        // ВСЁ УСТАЛ, ПОЙДУ В ARKNIGHTS ПОИГРАЮ И СПАТЬ
        // СПАСИБО МАТЕМАТИКЕ - ЭТО ОДИНАКОВО ЧТО ДЛЯ ЛЕВОЙ ЧТО ДЛЯ ПРАВОЙ ЧАСТИ
        let x_up   = vertex[prev].x,       y_up    = vertex[prev].y;
        let x_down = x,                    y_down  = y;
        let x_edge = to_optimize_lst[j].x, y_edge1 = to_optimize_lst[j].y, y_edge2 = maxY;
    ...
    console.log('>>> to_optimize_lst:', to_optimize_lst); 
    console.log('>>> cnt_chk_left: ', cnt_chk_left); 
    console.log('>>> cnt_chk_right:', cnt_chk_right); 



    console.log(`FINAL STEP (wtf)`);

    // (vvv) ЕЩЁ Я БЫ ХОТЕЛ ЧТОБ Y2 МЕНЯЛСЯ (vvv)
    // ПОКА ПО ТЕРПИШЬ!
    // А лол, хотя кажется оно само должно меняться?! Ааа, кажется вспомнил, надо всего начать использовать all_dots.y2

    // дорисовываем остатки (входящих рёбер)
    for(let xy of to_optimize_lst) {
        ...
        // ахпхпхп, ну и кринж (надо координаты только в all_dots хранить)
        let id_in_all = xy.id_in_all;
        let xx = all_dots[id_in_all].x, yy1 = all_dots[id_in_all].y1, yy2 = all_dots[id_in_all].y2;

        // вниз до необходимого уровня (при необходимости)
        draw_segment(xx, yy1, xx, yy2, COLORS.VERTEX_DRAGGING);
        
        // соединение в одну точку
        draw_segment(xx, yy2, x, y, COLORS.VERTEX_SELECTED);

        draw_circle(xx, yy1, 5, COLORS.VERTEX_SELECTED);
        
        draw_vertex_label(xy.from);
    }
    ...
    for(let j = 0; j < n; j++) {
        ...
        all_dots.push( {x: xj, y1: yj, y2: yj, comment: `mention ${nxt} from ${i}`} );  // пока dy = 0, длинна появится, когда эта ребро будет продленно вниз
