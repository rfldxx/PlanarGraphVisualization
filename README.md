Подумать на потом: зачем я меняю порядок потомков у детей? Можно ли обойтись без этого? Вроде это может аукнуться только в rotation при произвольных вызовах. Но ведь если делать общие вызовы, то там слетают и .i1 .i2 -> значит у нас точно должны быть "корректные" (aka топологическая сортировка) вызовы -> тогда как будто (когда настанет потом надо будет проверить) можно не поддерживать геометрический порядок потомков у вершин? \
Хотя такими темпами можно и к "префиксным" canvas докатится обо что точно бошку сломаешь (без вариантов на реализацию в коде).
